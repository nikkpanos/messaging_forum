...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if (empty($errors)){
        ...
        $approved = 1;
        $query = "insert into user(username, password, firstname, lastname, level, email, approved) values(?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($dbc, $query);
        mysqli_stmt_bind_param($stmt, 'ssssisi', $username, $password, $firstname, $lastname, $level, $email, $approved);
        mysqli_stmt_execute($stmt);
        print '<p class="text--success">Ο λογαριασμός δηιουργήθηκε επιτυχώς!</p>'; print "\n";
